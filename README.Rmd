...
<!-- badges: start -->
`r badger::badge_cran_checks("periglue")`
`r badger::badge_cran_download("periglue", "last-month", "green")`
[![DOI](https://zenodo.org/badge/291551786.svg)](https://zenodo.org/badge/latestdoi/291551786)
`r badger::badge_dependencies("periglue")`
<!-- badges: end -->
...
The goal of `periglue` is to give you more control over the presentation of your data and also simplify the process of rounding, formatting, and presenting your data. The main idea is to create rounding specifications (starting with `round_spec()`) that can be plugged in, directly or through global options, to the `peri_glue()` and `peri_value()` functions. 

## Installation

You can install the released version of periglue from [CRAN](https://CRAN.R-project.org) with:
...
Suppose we want to write a summary statement about the mean and standard deviation (SD) of a column in the `mtcars` data. We'll start by loading the `glue`, `periglue`, and `magrittr` packages.
...
This is certainly more readable and clean. The only thing `glue()` doesn't do is make the pretty versions of `col_mean` and `col_sd`. This is where `periglue` comes in.

### Use `peri_glue()`

The `periglue` package lets you use `glue()` without having to make numbers pretty beforehand. For example, the code below uses `peri_glue()`, one of the main functions in `periglue`, to replicate the results we got from `glue()` but without using the pretty versions of `col_mean` and `col_sd`. 
...
- `peri_glue` applies a general rounding convention to all numeric data that are supplied in the input string. 
...
rounding specifications can also be passed into global options so that `peri_glue()` and `peri_value()` will use the specification automatically (see the [Setting a default rounding specification](https://bcjaeger.github.io/periglue/articles/default_rounder.html) vignette)
