...
test_that("missing values are rejected", {
  ...
  expect_error(peri_ester(estimate = NA, error = 1),
               regexp = 'Missing values')

  expect_error(peri_estin(estimate = NA, lower = 1, upper = 2),
               regexp = 'Missing values')
...
test_that("type errors are rejected", {
  ...
  expect_error(peri_ester(estimate = "1", error = 1),
...
test_that('length errors are rejected', {
  ...
  expect_error(
    peri_ester(estimate = c(1,2), error = c(1)),
    'do not match'
  )

  expect_error(
    peri_estin(estimate = c(1,2), lower = c(1), upper = c(2,3)),
...
test_that("examples are correct", {
  ...
  example_one <- peri_ester(estimate = 72.17986,
                             error = 9.364132,
                             form = "{estimate} ({error})")

  example_two <- peri_ester(estimate = 72347.23,
                             error = 23994.06,
                             form = "{estimate} ({error})")
...
test_that("estimate intervals example is correct",{
  ...
  expect_equal(
    peri_estin(estimate = 72.345, lower = 62.345, upper = 82.345),
    "72 (62, 82)"
  )
...
test_that('majority rule works', {
  ...
  expect_equal(
    peri_ester(estimate = c(72.17986, 72.17986, 72347.23),
  ...
  expect_equal(
    peri_estin(estimate = c(72.17986, 72.17986, 72347.23),
