...
#'   `peri_pvalue()` is meant to do almost all of the finessing for you.
...
peri_pvalue <- function(x,
  ...
  #   out[x_in_lower_bound] <- peri_value(x, rspec_lwr)[x_in_lower_bound]
  #
  # if ( any(x_in_middle) )
  #   out[x_in_middle] <- peri_value(x, rspec_mid)[x_in_middle]
  #
  # if( any(x_in_upper_bound) )
  #   out[x_in_upper_bound] <- peri_value(x, rspec_upr)[x_in_upper_bound]
  ...
  while( any(out %in% yellow_vals & x < alpha) ) {
    ...
    out[x_in_yellow_zone] <- peri_value(x[x_in_yellow_zone], rspec_yellow)
