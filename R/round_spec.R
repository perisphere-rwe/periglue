...
#'
#'   Rounding specifications are meant to be passed into the [peri_glue]
#'   and [peri_value] functions. A session-wide default specification can be
#'   stored with [set_default_rounding_spec()], which `round_spec()` will
#'   use automatically when `force_default = FALSE`.
#'
...
#'   ignores any stored default (see [set_default_rounding_spec]) and global
#'   options, returning factory default values. If `FALSE`, `round_spec()`
#'   first checks for a stored default, then falls back to `periglue.*`
#'   global options.
#'
...
round_spec <- function(force_default = FALSE) {
  ...
  if(!force_default) {
    ...
    # A stored default (set via set_default_rounding_spec()) takes precedence
    # over individual periglue.* options.
    if (!is.null(.periglue_internal$rspec)) return(.periglue_internal$rspec)
    ...
    rspec <- list(
      ...
      big_interval   = getOption("periglue.big_interval",     default = 3L),
      small_mark     = getOption("periglue.small_mark",       default = ''),
      small_interval = getOption("periglue.small_interval",   default = 5L),
      decimal_mark   = getOption("periglue.decimal_mark",     default = '.'),
      zero_print     = getOption("periglue.zero_print",       default = NULL)
    )
  }
...
format_decimal <- function(rspec, mark = '.'){
  ...
  if (nchar(mark) != 1)
    stop("`mark` must be a single character, but '", mark, "' has ",
         nchar(mark), " characters.", call. = FALSE)
...
#'   can be used for many different applications (e.g., see [peri_pvalue]).
