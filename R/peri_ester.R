...
#'  over the number of decimals shown, they should use [peri_glue()]
...
peri_ester <- function(estimate,
  ...
  .estimate <- .error <- rep(
    x = getOption('periglue.miss_replace', default = '--'),
    times = length(estimate)
  )

  big.mark       = getOption("periglue.big_mark",       default = ',')
  big.interval   = getOption("periglue.big_interval",   default = 3L)
  small.mark     = getOption("periglue.small_mark",     default = '')
  small.interval = getOption("periglue.small_interval", default = 5L)
  decimal.mark   = getOption("periglue.decimal_mark",   default = '.')
  zero.print     = getOption("periglue.zero_print",     default = NULL)
